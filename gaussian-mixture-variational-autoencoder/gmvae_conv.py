...
class P_x(nn.Module):
    ...
    def __init__(self, input_features = 1024, latent_features = 64):
        super(P_x, self).__init__()   
        self.affine_dec1 = nn.Linear(latent_features, 256, bias=False)
        self.affine_dec2 = nn.Linear(256, (((32-4)//2-4)//2-4//2)**2*32, bias=False)
        
        self.conv1 = nn.ConvTranspose2d(in_channels=16, out_channels=1, kernel_size=4, stride=2, padding=1, bias=False)
        self.conv2 = nn.ConvTranspose2d(in_channels=32, out_channels=16, kernel_size=4, stride=2, padding=1, bias=False)
        self.conv3 = nn.ConvTranspose2d(in_channels=32, out_channels=32, kernel_size=4, stride=2, padding=0, bias=False)
        
    def forward(self, z):
        h1 = F.relu(self.affine_dec1(z))
        h2 = F.relu(self.affine_dec2(h1))
        h2 = h2.view(-1, 32, (h2.size(1)/32)**0.5, (h2.size(1)/32)**0.5)
        
        px = F.relu(self.conv3(h2))
        px = F.relu(self.conv2(px))
        px = self.conv1(px)
        return px
...
class Q_y(nn.Module):
    ...
    def __init__(self, input_features = 1024, mixtures_count = 10):
        super(Q_y, self).__init__()   
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=32, kernel_size=5, stride=1, padding=0, bias=False)
        self.pool1 = nn.MaxPool2d(2)
        self.conv2 = nn.Conv2d(in_channels=32, out_channels=128, kernel_size=5, stride=1, padding=0, bias=False)
        self.pool2 = nn.MaxPool2d(2)
        self.conv3 = nn.Conv2d(in_channels=128, out_channels=128, kernel_size=5, stride=1, padding=0, bias=False)
        
        self.flattened_features = (((32-4)//2-4)//2-4)**2*128
        
        self.affine_prop1 = nn.Linear(self.flattened_features, 256, bias=False)
        self.affine_prop2 = nn.Linear(256, 256, bias=False)
        self.affine_prop3 = nn.Linear(256, mixtures_count)
...
class Q_z(nn.Module):
    ...
    def __init__(self, input_features = 1024, latent_features = 64, mixtures_count = 10):
        ...
        self.affine_enc1 = nn.Linear(self.flattened_features + mixtures_count, 256, bias=False)
        self.affine_enc2 = nn.Linear(256, 256, bias=False)
        self.affine_enc3 = nn.Linear(256, latent_features)
        self.affine_enc4 = nn.Linear(256, latent_features)
...
class GMVAE(nn.Module):
    ...
    def __init__(self, input_features = 1024, latent_features = 128, mixtures_count = 20):
...
class NegVariationalLowerBound(nn.Module):
     ...
     def forward(self, netoutput, netinput):
        ...
        # Unpack output data
        x_recon = [cluster_output.view(netinput.size(0), -1) for cluster_output in netoutput]
        ...
        qy, qy_linear = self.vae.qy(netinput)
