...
if __name__ == '__main__':
    ...
    # WOOD
    transform = transform=transforms.Compose([transforms.ToTensor(), helpers.flatten_transform])
    wood_data = ImagePatches("/ext/data/Wood-0035.png", mode='gray', train=True, stride_y=14, stride_x=14, window_size=28, transform=transform)
    wood_data_test = AnomalyImagePatches("/ext/data/Wood-0035.png", mode='rgb', train=False, stride_y=14, stride_x=14, window_size=28, transform=transform, propability=1.0, anomaly=SquareAnomaly(20, color=255))
    
    wood_data = [wood_data[i][0] for i in range(len(wood_data))]
    wood_data = torch.stack(wood_data).numpy()
    
    wood_data_test  = [wood_data_test[i][0] for i in range(len(wood_data_test))]
    wood_data_test  = torch.stack(wood_data_test).numpy()
...
######################################################
    ...
    with Logger(loglevel=10, log_batch_interval=1) as lg:
...
######################################################
    ...
    # LOSS        
    criterion = KLLoss_laplace(model=model, size_average=False, mean=2, scale=0.3)
...
######################################################
    ...
    class CombinedLoss(nn.Module):
        ...
        def __init__(self, model, *args, **kwargs):
            ...
            self.kl_loss = KLLoss_laplace(model=model, size_average=False, mean=1, scale=0.3)
    ...
    # Model    
    model = ModelLaplace(input_features = data_train.shape[1], hidden_layer=50, latent_features=20)
